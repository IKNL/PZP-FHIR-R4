...
def generate_integration_json(zib2017, stu3_mappings, zib2020, r4_mappings, output_file):
    ...
    """
    # Load flat profile mappings generated by extract_all_profile_mappings.py
    flat_file = Path(__file__).parent / 'all_profile_mappings.json'
    flat_index = {}
    if flat_file.exists():
        try:
            ...
            print(f"Warning: failed to load flat mappings: {e}")
    else:
        print(f"Flat profile mappings file not found: {flat_file}")
        ...
        for fn in os.listdir(ig_res_dir):
            if fn.endswith('.json'):
                try:
                    ...
                    continue
    # also load core Nictiz STU3 profiles for datatype mappings
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        for did in dataset_order:
            ...
            for m in mappings:
                ...
                # only include ACP IG resources and allowed datatypes
                if resource not in in_scope:
                    continue
                f.write(f"| {did} | {dname} | {resource} | `{m.get('element_path', '')}` |\n")
